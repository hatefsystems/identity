...
- **Hard Delete Cron Jobs:** Once the 30-day grace window expires, a Go worker (Cron Job) is scheduled to permanently purge soft-deleted records (`pending_deletion` users where `deleted_at < NOW() - INTERVAL '30 days'`) from PostgreSQL. This completely and physically wipes the user's Class A records across all tables (ACID transactions), satisfying "Right to be Forgotten" mandates.
